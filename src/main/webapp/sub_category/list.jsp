    ...
    $.ajax({
      ...
      success: function(rdata) { // callback 함수
        ...
        for(index=0; index < rdata.length; index++) {
          ...
          panel += "<TD><A href='../contents/list_by_category_paging.do?s_categoryno="+rdata[index].s_categoryno+"'>"+rdata[index].title+"</A></TD>";
          ...
          panel += "  <A href='../contents/create.do?s_categoryno="+rdata[index].s_categoryno+"'><IMG src='./images/create.png' title='등록'></A>";
  ...
  function update(s_categoryno) {
    ...
    $.ajax({
      ...
      success: function(rdata) { // callback 함수
        ...
        $('#m_categrpno', frm_update).val(rdata.m_categrpno); // SELECT tag
...
<DIV class='content' style='width: 100%;'>
  ...
  <DIV class='title_line'><a href="../main_categrp/list.do"> 메인 카테그룹 목록</a>
  > 전체 카테고리</DIV>
  ...
  <FORM name='frm_seqno' id='frm_seqno' method='post' action=''>
    <input type='hidden' name='m_categrpno' id='m_categrpno' value='${categrpVO.m_categrpno }'>
    <input type='hidden' name='s_categoryno' id='s_categoryno' value=''>
  </FORM>
  ...
  <DIV id='panel_update' style='padding: 10px 0px 10px 0px; background-color: #DDDDDD; width: 100%; text-align: center;'>  
    <FORM name='frm_update' id='frm_update' method='POST' action='./update.do'>
      ...
      <label for='title'>메인 카테고리</label>
       <select name='m_categrpno' id='m_categrpno'>
         <c:forEach var="m_categrpVO" items="${m_categrp_list}">
           <option value='${categrpVO.m_categrpno }'>${categrpVO.name}</option>
         </c:forEach>
       </select>
